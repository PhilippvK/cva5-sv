//TODO: License header (based on CVA5, Apache License 2.0 / )
...
interface scaiev_interface; //Stage valid signals refer to any instruction.
    ...
    logic fetch_valid;
    logic [LOG2_MAX_IDS-1:0] fetch_fetchID; //The assigned fetch ID
    ...
    logic fetch_flush;
    logic [LOG2_MAX_IDS-1:0] fetch_fetchFlushID; //The oldest fetch ID to flush if flushing
    ...
    logic [LOG2_MAX_IDS-1:0] decode_fetchID; //The fetch ID corresponding to the current instruction
    ...
    logic [LOG2_MAX_IDS-1:0] execute_ID; //The issue ID corresponding to the current instruction, or a fake counter
    logic execute_expects_rd; //If set, the execute ID is the actual issue ID; otherwise, it is a fake counter
    logic execute_valid;
    //REQUIRES active issue_stall from first cycle where ISAX entered execute.
    ...
    modport core (input fetch_stall, fetch_flush, fetch_wrPC, fetch_wrPCValid,
            ...
            issue_stall, issue_flush, issue_injectLS_potentiallyValid, issue_injectLS_valid, issue_injectLS_rnw, issue_injectLS_relaxedOrdering, issue_injectLS_addr, issue_injectLS_storeVal,
            execute_flush, execute_stall, execute_wrRD, execute_wrRD_data, execute_deq, execute_commitNow, execute_commitNow_ID, execute_commitNow_expects_rd, execute_injectLS_potentiallyValid, execute_injectLS_valid, execute_injectLS_rnw, execute_injectLS_relaxedOrdering, execute_injectLS_addr, execute_injectLS_storeVal,
            rf_wrReg, rf_wrReg_phys_RD, rf_wrReg_prev_phys_RD, rf_wrReg_data, rf_isDecoupledWB, renamer_addtofree_skip,
            output pre_fetch_PC, pre_fetch_isStalling, fetch_PC, fetch_valid, fetch_fetchID, fetch_isStalling, fetch_isFlushing, fetch_isFlushing_internal, fetch_fetchFlushID,
            decode_PC, decode_Instr, decode_valid, decode_fetchID, decode_isStalling, decode_isFlushing, decode_isFlushing_internal,
            decode_RS1_valid, decode_RS1_id, decode_RS2_valid, decode_RS2_id, decode_RD_AS_RS_valid, decode_RD_AS_RS_id, decode_RD_valid, decode_RD_id,
            issue_PC, issue_Instr, issue_RS1_valid, issue_RS1, issue_RS2_valid, issue_RS2, issue_RD_AS_RS_valid, issue_RD_AS_RS, issue_RD_valid, issue_RD_id, issue_phys_RD_decoupled, issue_prev_phys_RD_decoupled,
            ...
            execute_PC, execute_Instr, execute_RS1, execute_RS2, execute_RD_AS_RS, execute_ID, execute_expects_rd, execute_valid, execute_isStalling, execute_isStalling_reason_isax, execute_isStalling_reason_core, execute_commitNow_resp, execute_injectLS_done,
    ...
    modport scal (output fetch_stall, fetch_flush, fetch_wrPC, fetch_wrPCValid,
            ...
            execute_flush, execute_stall, execute_wrRD, execute_wrRD_data, execute_deq, execute_commitNow, execute_commitNow_ID, execute_commitNow_expects_rd, execute_injectLS_potentiallyValid, execute_injectLS_valid, execute_injectLS_rnw, execute_injectLS_relaxedOrdering, execute_injectLS_addr, execute_injectLS_storeVal,
            rf_wrReg, rf_wrReg_phys_RD, rf_wrReg_prev_phys_RD, rf_wrReg_data, rf_isDecoupledWB, renamer_addtofree_skip,
            input pre_fetch_PC, pre_fetch_isStalling, fetch_PC, fetch_valid, fetch_fetchID, fetch_isStalling, fetch_isFlushing, fetch_isFlushing_internal, fetch_fetchFlushID,
            decode_PC, decode_Instr, decode_valid, decode_fetchID, decode_isStalling, decode_isFlushing, decode_isFlushing_internal,
            decode_RS1_valid, decode_RS1_id, decode_RS2_valid, decode_RS2_id, decode_RD_AS_RS_valid, decode_RD_AS_RS_id, decode_RD_valid, decode_RD_id,
            issue_PC, issue_Instr, issue_RS1_valid, issue_RS1, issue_RS2_valid, issue_RS2, issue_RD_AS_RS_valid, issue_RD_AS_RS, issue_RD_valid, issue_RD_id, issue_phys_RD_decoupled, issue_prev_phys_RD_decoupled,
            issue_valid, issue_isSCAIEV, issue_isLS, issue_injected, issue_isStalling, issue_isFlushing, issue_isFlushing_internal, issue_injectLS_done,
    ...
    modport trace_out (output fetch_stall, fetch_flush, fetch_wrPC, fetch_wrPCValid,
            ...
            execute_flush, execute_stall, execute_wrRD, execute_wrRD_data, execute_deq, execute_commitNow, execute_commitNow_ID, execute_commitNow_expects_rd, execute_injectLS_potentiallyValid, execute_injectLS_valid, execute_injectLS_rnw, execute_injectLS_relaxedOrdering, execute_injectLS_addr, execute_injectLS_storeVal,
            rf_wrReg, rf_wrReg_phys_RD, rf_wrReg_prev_phys_RD, rf_wrReg_data, rf_isDecoupledWB, renamer_addtofree_skip,
            pre_fetch_PC, pre_fetch_isStalling, fetch_PC, fetch_valid, fetch_fetchID, fetch_isStalling, fetch_isFlushing, fetch_isFlushing_internal, fetch_fetchFlushID,
            decode_PC, decode_Instr, decode_valid, decode_fetchID, decode_isStalling, decode_isFlushing, decode_isFlushing_internal,
            decode_RS1_valid, decode_RS1_id, decode_RS2_valid, decode_RS2_id, decode_RD_AS_RS_valid, decode_RD_AS_RS_id, decode_RD_valid, decode_RD_id,
            issue_PC, issue_Instr, issue_RS1_valid, issue_RS1, issue_RS2_valid, issue_RS2, issue_RD_AS_RS_valid, issue_RD_AS_RS, issue_RD_valid, issue_RD_id, issue_phys_RD_decoupled, issue_prev_phys_RD_decoupled,
            issue_valid, issue_isSCAIEV, issue_isLS, issue_injected, issue_isStalling, issue_isFlushing, issue_isFlushing_internal, issue_injectLS_done,
